...
main_categories <- main_page %>%
  ...
  str_replace("/1$", "/50") %>%
  discard(is.na) %>%
  str_subset("^/(vestimenta|calzado|carteras-y-bolsos|joyeria-y-bijou)/50$")

main_categories[c(1:4)]
...
scrape_images_subcategory <- function(subcat_url) {
  ...
  # Create a folder name based on the subcategory path
  subcat_name <- str_replace_all(subcat_url, "/", "_")
  folder_path <- file.path("images-previa", subcat_name)
  dir_create(folder_path)
  
  # Download each image into the subcategory folder
  walk2(image_links, seq_along(image_links), function(img_link, i) {
    
    if (!is.na(img_link) && img_link != "") {
      img_name <- paste0("img_", i, ".jpg")
