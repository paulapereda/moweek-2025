...
category_urls <- lapply(category_nodes, function(node) html_nodes(node, "a") %>%
                          html_attr("href")) %>%
  unlist() %>%
  str_subset("/joyeria-y-bijou/") %>%
  str_replace_all("1", "100")

category_urls <- category_urls[!(category_urls %in% c("/joyeria-y-bijou/100",
                                                      "/joyeria-y-bijou/100?filters=10070",                
                                                      "/joyeria-y-bijou/100?filters=1007100",
                                                      "https://moweek.com.uy/joyeria-y-bijou/collares/100"))]
...
write_rds(product_data, "data-raw/product_jewelry.rds")
...
product_data_clean <- product_data %>%
  ...
  mutate(price = as.numeric(str_replace_all(price, "[\\$\\s.]", "")),
         ...
         type = "Joyería y bijou")

write_rds(product_data_clean, here("data-raw", "product_jewelry_clean.rds"))
