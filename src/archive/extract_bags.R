...
category_urls <- lapply(category_nodes, function(node) html_nodes(node, "a") %>%
                          html_attr("href")) %>%
  unlist() %>%
  str_subset("/carteras-y-bolsos/") %>%
  str_replace_all("1", "100")

category_urls <- category_urls[!(category_urls %in% c("/carteras-y-bolsos/100",
                                                      "/carteras-y-bolsos/otros-materiales/100",
                                                      "/carteras-y-bolsos/100?filters=10070",                
                                                      "/carteras-y-bolsos/100?filters=1007100",
                                                      "https://moweek.com.uy/carteras-y-bolsos/carteras/100"))]
...
write_rds(product_data, "data-raw/product_bags.rds")
...
product_data_clean <- product_data %>%
  ...
  mutate(price = as.numeric(str_replace_all(price, "[\\$\\s.]", "")),
         ...
         type = "Carteras y bolsos")

write_rds(product_data_clean, here("data-raw", "product_bags_clean.rds"))
